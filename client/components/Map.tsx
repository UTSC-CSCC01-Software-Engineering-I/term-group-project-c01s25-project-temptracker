...
const Map = (props: MapProps) => {
  ...
  const [interpolatedTempData, setInterpolatedTempData] = useState<TemperaturePoint[]>(() => {
  ...
  const [geoGrids, setGeoGrids] = useState(() => {
    ...
    const [mapCoords, setMapCoords] = useState(()=> {
      if (props.centerLatitude != null && props.centerLongitude != null) {
        return {latitude: props.centerLatitude, longitude: props.centerLongitude}
      } else {
  ...
  useEffect(() => {
    localStorage.setItem("INTERPOLATED_TEMP_DATA", JSON.stringify(interpolatedTempData));
...
const SliderLayer = () => {
  ...
  const monthTime = new Date(Date.now() - (30 - currentMonthDate) * 24 * 60 * 60 * 1000)
  const weekTime = new Date(Date.now() - (7 - currentWeekday) * 24 * 60 * 60 * 1000)
  ...
  if (props.timeRange === "week") {
    ...
    return (
      ...
      >
        <h2 className="slider-title" style={{ color: 'white', textAlign: 'center', marginBottom: '15px' }}>
          {`${weekTime.toLocaleString('en-US', { month: 'long' })} ${weekTime.getDate()} ${weekTime.getFullYear()}`}
        </h2>
        <Slider 
    ...
    return (
      ...
      >
        <h2 className="slider-title" style={{ color: 'white', textAlign: 'center', marginBottom: '15px' }}>
          {`${monthTime.toLocaleString('en-US', { month: 'long' })} ${monthTime.getDate()} ${monthTime.getFullYear()}`}
        </h2>
        <Slider 
    ...
    useEffect(() => {
      ...
      const validData = data.filter(point => {
        ...
        if (typeof lat !== 'number' || typeof lng !== 'number' || 
          ...
          console.warn('Invalid coordinates or intensity:', { lat, lng, intensity });
          return false;
        }
        return true
      })
  ...
  const findNearestTemperaturePoint = (
    ...
    const interpolation = (polygon: any, tempPoints: any, point: any) => {
      ...
      for (let i = 0; i < polygonPoints.length; i++) {
        ...
        if (dist === 0) {
          const newPoint: TemperaturePoint = [point[1], point[0], polygonPoints[i][2]];
          return newPoint
        }
        ...
        if (props.timeRange == "week") {
          ...
          for (let k=0; k<weekData.length; k++) {
            ...
            for (let i=0; i<geoGrids.length; i++) {
                ...
                const tempPoint = interpolation(geoGrids[i]["polygon"].coordinates, [...weekDataWithoutTime, ...allNewPoints], point);
          ...
          for (let k=0; k<monthData.length; k++) {
            ...
            for (let i=0; i<geoGrids.length; i++) {
                ...
                for (let j = 0; j < geoGrids[i]["grid"].length; j++) {
                const point = geoGrids[i]["grid"][j];
                const tempPoint = interpolation(geoGrids[i]["polygon"].coordinates, [...monthDataWithoutTime, ...allNewPoints], point);
                allNewPoints.push(tempPoint);
              }
            }
          ...
          for (let i=0; i<geoGrids.length; i++) {
              for (let j = 0; j < geoGrids[i]["grid"].length; j++) {
              const point = geoGrids[i]["grid"][j];
              const tempPoint = interpolation(geoGrids[i]["polygon"].coordinates, [...tempData, ...allNewPoints], point);
              allNewPoints.push(tempPoint);
            }
          }   
          ...
          if (allNewPoints.length > 0) {
            ...
            const join: TemperaturePoint[] = [...tempData, ...allNewPoints].map((point) => {
    ...
    if (mapCoords.latitude && mapCoords.longitude && geoGrids.length > 0 && !loading) {
        return (
            <MapContainer key={`${mapCoords.latitude},${mapCoords.longitude}`} center={[mapCoords.latitude, mapCoords.longitude]} zoom={13}>
                <TileLayer
                ...
                />
                <MapClickHandler />
                {tempVisible && props.timeRange == "all" && interpolatedTempData.length >0 &&<HeatmapLayer data={interpolatedTempData} />}
                {tempVisible && props.timeRange == "week" && weekDataBucket.length >0 && weekDataBucket[currentWeekday-1].length > 0 && <HeatmapLayer data={weekDataBucket[currentWeekday-1]} />}
                {tempVisible && props.timeRange == "month" && monthDataBucket.length > 0 && monthDataBucket[currentMonthDate-1].length > 0 && <HeatmapLayer data={monthDataBucket[currentMonthDate-1]} />}
