...
5. Add map options for colour-coded data based on the temperature. Data Points on the map create a coloured area around them according to a gradient scale and 6. allow users to interact to see the current temperature. Also includes a legend explaining the scale.
Setup directories to make a scalable base for the project, we separated client and server and made subdirectories to make it easy to scale the project
